...
select localidad, nombre, indice_de_ocupacion from
  (select 
    (cast(plazas_ocupadas as float)/plazas_ofertadas) as indice_de_ocupacion 
    , estudio.nombre as nombre
    , centro.localidad as localidad
    , RANK() OVER (PARTITION BY localidad ORDER BY (cast(plazas_ocupadas as float)/plazas_ofertadas) desc) as ranking
